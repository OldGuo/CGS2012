...
/**
 * @author Young More specific type of InteractButton Has right and wrong answers
 */
public class QuestionButton extends InteractButton {
	...
	public QuestionButton(String answer, int xPos, int yPos, int l, int h, int id) throws SlickException {
		...
		buttonID = id;
		correct = false;
		buttonWrong = new Image("data" + GameConstants.separatorChar + "Questions" + GameConstants.separatorChar
				+ "QuestionButtonWrong.png");
	}
	...
	public void update(GameContainer gc, Input input, int correctAnswer) {
		...
		if ((input.getMouseX() > getX() && input.getMouseX() < getX() + getLength() && input.getMouseY() > getY()
				&& input.getMouseY() < getY() + getHeight()) || selectionState == 0) {
			if ((input.isMousePressed(Input.MOUSE_LEFT_BUTTON) || (selectionState == 0 && input.isKeyDown(Input.KEY_SPACE) && delay <= 0))
					&& wrongPressed == false) {
				setClick(true);
